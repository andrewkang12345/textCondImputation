...
def make_gif(court_png, traj_gt, traj_pred, mask_bool, masked_agent, caption, out_path,
    ...
    TEAM_COLORS = [
        ...
        (255, 140, 0, 230),   # Team 2: DarkOrange
        (34, 139, 34, 230),   # Team 3: ForestGreen
    ...
    COLOR_TEXT=(20,20,20,255); R_OTHER=8; R_MASK=10
    ...
    for t in range(T):
        ...
        # Dots for non-masked agents (team-colored)
        for a in range(A):
            if a==masked_agent: continue
        ...
        # Masked agent: ring (GT) and filled (PRED)
        _draw_circle(draw, gt_px[t,masked_agent],   R_MASK, fill=None, outline=COLOR_GT, width=3)
...
def main():
    ...
    ap.add_argument("--checkpoint", default="outputs/diffusion_mid/best.pt")
    ...
    # teams
    ap.add_argument("--team_assign", type=str, default=None,
...
#     COLOR_OTHER=(180,180,180,255); COLOR_OTHER_TRAIL=(180,180,180,140)
#     COLOR_GT=(255,102,0,230); COLOR_PRED=(66,133,244,230)
#     COLOR_GT_TRAIL=(255,102,0,140); COLOR_PRED_TRAIL=(66,133,244,140)
#     COLOR_TEXT=(20,20,20,255); R_OTHER=8; R_MASK=10
#     font=_try_font(); frames=[]
...
#             if a==masked_agent: continue
#             for k in range(t0,t):
#                 _draw_line(draw, gt_px[k,a], gt_px[k+1,a], COLOR_OTHER_TRAIL, 2)
#         for k in range(t0,t):
#             _draw_line(draw, gt_px[k,masked_agent],   gt_px[k+1,masked_agent],   COLOR_GT_TRAIL, 4)
#             _draw_line(draw, pred_px[k,masked_agent], pred_px[k+1,masked_agent], COLOR_PRED_TRAIL, 4)
#         for a in range(A):
#             if a==masked_agent: continue
#             _draw_circle(draw, gt_px[t,a], R_OTHER, fill=COLOR_OTHER)
#             if show_ids: draw.text((gt_px[t,a,0]+6, gt_px[t,a,1]-6), str(a), fill=COLOR_TEXT, font=font)
#         _draw_circle(draw, gt_px[t,masked_agent],   R_MASK, fill=None, outline=COLOR_GT, width=3)
#         _draw_circle(draw, pred_px[t,masked_agent], R_MASK, fill=COLOR_PRED, outline=None)
#         lines=[f"t={t+1}/{T}", f"masked agent={masked_agent}",
#                f"{'masked' if mask_bool[t,masked_agent] else 'observed'} at t",
